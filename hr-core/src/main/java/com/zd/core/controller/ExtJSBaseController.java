...
import com.zd.core.web.CustomDateEditor;
...
public abstract class ExtJSBaseController<E extends ExtJSBaseParameter> {
    ...
    public void doContent(@PathVariable("id") Serializable id, HttpServletRequest request, HttpServletResponse response)
            throws IOException {
        Class c = (Class) ((java.lang.reflect.ParameterizedType) super.getClass().getGenericSuperclass())
                .getActualTypeArguments()[0];
        Class returnClass = null;
        try {
            returnClass = c.getMethod("get" + this.idField.substring(0, 1).toUpperCase() + this.idField.substring(1),
                    new Class[0]).getReturnType();
        } catch (Exception e) {
            e.printStackTrace();
        }
        ExtJSBaseParameter entity = (ExtJSBaseParameter) this.service
                .get((Serializable) ConvertUtils.convert(id, returnClass));
