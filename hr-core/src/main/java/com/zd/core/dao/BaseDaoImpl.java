...
public class BaseDaoImpl<E> implements BaseDao<E> {
    ...
    protected Map handlerConditions(BaseParameter param) throws Exception {
        Map staticConditions = com.zd.core.util.BeanUtils.describe(param);
	...
	private void processQuery(Criteria criteria, BaseParameter param) {
		try {
			Map<String, Object> staticConditionMap = com.zd.core.util.BeanUtils.describeAvailableParameter(param);
			...
			if ((dynamicConditionMap != null) && (dynamicConditionMap.size() > 0)) {
				...
				for (Map.Entry e : dynamicConditionMap.entrySet()) {
					...
					String propName = getPropName(pn);
					...
					if ((value != null) && (((!(value instanceof String)) || (!("".equals((String) value))))))
						if ("like".equals(methodName))
							criteria.add((Criterion) m.invoke(Restrictions.class,
									new Object[] { propName, value, MatchMode.ANYWHERE }));
						else if (("isNull".equals(methodName)) && (value instanceof Boolean)) {
							if (((Boolean) value).booleanValue())
							...
							criteria.add((Criterion) m.invoke(Restrictions.class, new Object[] { propName, value }));
				}
			}
		} catch (Exception e) {
			e.printStackTrace();
