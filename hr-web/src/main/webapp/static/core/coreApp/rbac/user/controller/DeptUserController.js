		...
		this.control({
			...
			"panel[xtype=rbac.depttree] button[ref=treeIns]": {
				click: function(btn) {
					...
					var resObj = self.ajax({
						url: "/rbac/deptAction!doSave.action",
						params: params
					});
					if (resObj.success) {
			...
			"panel[xtype=rbac.depttree] button[ref=treeDel]": {
				click: function(btn) {
					...
					var node = records[0];
					var resObj = self.ajax({
						url: "/rbac/deptAction!doRemove.action",
						params: {
							ids: node.get("id")
						}
					});
			...
			"panel[xtype=rbac.deptform] button[ref=submit]": {
				click: function(btn) {
					...
					if (params.deptId != null && params.deptId != "") {
						var resObj = self.ajax({
							url: "/rbac/deptAction!doUpdate.action",
							params: params
						});
			...
			"panel[xtype=rbac.usergrid] button[ref=gridInsertF]": {
				beforeclick: function(btn) {
					var baseGrid = btn.up("panel[xtype=rbac.usergrid]");

					//选择的部门信息
					var deptTree = baseGrid.up("panel[xtype=rbac.mainlayout]").down("panel[xtype=rbac.depttree]");
					...
					baseForm.show();
					baseForm.fireEvent("activate", baseForm);
					//执行回调函数						
					if (btn.callback) {
						btn.callback();
					}
