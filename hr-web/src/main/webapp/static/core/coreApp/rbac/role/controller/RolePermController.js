Ext.define("core.rbac.role.controller.RolePermController",{
	...
	init:function(){
		...
		self.setPerm=function(tree,records,roleId){
			...
			var resObj=self.ajax({url:"/rbac/permAction!updatePerm.action",params:{roleId:roleId,addIds:addIds.join(","),delIds:delIds.join(",")}});
			if(resObj.success){
				self.msgbox("授权成功");
			}else{
				alert(resObj.obj);
		...
		this.control({
			...
			"panel[xtype=role.roletree] button[ref=treeIns]":{
				click:function(btn){
					...
					var resObj=self.ajax({url:"/rbac/roleAction!doSave.action",params:params});
					if(resObj.success){
			...
			"panel[xtype=role.roleform] button[ref=submit]":{
				click:function(btn){
					...
					if(params.roleId!=null && params.roleId!=""){
						var resObj=self.ajax({url:"/rbac/roleAction!doUpdate.action",params:params});						
						if(resObj.success){
			...
			"panel[xtype=role.roletree] button[ref=treeDel]":{
				click:function(btn){
					...
					var node=records[0];
					var resObj=self.ajax({url:"/rbac/roleAction!doRemove.action",params:{ids:node.get("id")}});
					if(resObj.success){
						tree.getStore().load();
			...
			"panel[xtype=role.usergrid] button[ref=addUser]":{
				click:function(btn){
					...
					self.selTreeWin({
						...
						isEmpty:false,
						config:{
							url:"/rbac/deptUserAction!getTree.action",
							params:{
								whereSql:" and 1=1",
								expanded:true
							}
						...
						callback:function(win,records){
							...
							var resObj=self.ajax({url:"/rbac/roleAction!addUsers.action",params:{roleId:role.get("id"),ids:ids.join(",")}});
								if(resObj.success){
			...
			"panel[xtype=role.usergrid] button[ref=removeUser]":{
				click:function(btn){
					...
					var resObj=self.ajax({url:"/rbac/roleAction!removeUsers.action",params:{roleId:role.get("id"),ids:ids.join(",")}});
					if(resObj.success){
						var proxy=userGrid.getStore().getProxy();
						proxy.extraParams={
								roleId:role.get("id")
			...
			"panel[xtype=role.moduletree]":{
				itemdblclick:function(tree,record){
					...
					self.selTreeWin({
						...
						isEmpty:true,
						config:{
							url:"/rbac/permAction!getPermTree.action",
							params:{
								whereSql:" and 1=1",
								isSee:false,								
								roleId:role.get("id"),
